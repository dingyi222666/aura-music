...
/** Immutable grid topology; the production vertex shader derives UVs from IDs. */
export const triangles = (side = SIDE) => {
  const indices = new Uint16Array((side - 1) * (side - 1) * 6);
  // Nearby triangles reuse transformed vertices in the GPU's small cache.
  // Full-width rows evicted most of the next row before it was drawn.
  // The geometry/winding stays identical; higher-u tiles are drawn first so
  // the existing depth test can also reject hidden parts of a local fold.
  let offset = 0;
  for (let right = side - 1; right > 0; right -= 8) {
    for (let top = 0; top < side - 1; top += 8) {
      for (let y = top; y < Math.min(top + 8, side - 1); y++) {
        for (let x = right - 1; x >= Math.max(0, right - 8); x--) {
          const p = y * side + x;
          indices.set([p, p + 1, p + side, p + 1, p + side + 1, p + side], offset);
          offset += 6;
        }
      }
    }
