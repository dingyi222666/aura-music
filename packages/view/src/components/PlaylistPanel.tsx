...
const Art = React.memo(({ src, alt, dim = false, eager = false }: ArtProps) => {
  if (!src) {
    return (
      <div className="relative h-11 w-11 shrink-0 overflow-hidden rounded-lg border border-white/5 bg-gray-800 shadow-xs">
...
const PlaylistPanel = React.memo(({
    ...
    useEffect(() => {
        if (isOpen) return;
        const timer = window.setTimeout(() => { setIsEditing(false); setSelectedIds(new Set()); }, 380);
    ...
    useEffect(() => {
        const handleClickOutside = (event: MouseEvent) => {
            if (isOpen && !isAdding && panelRef.current && !panelRef.current.contains(event.target as Node) && !(event.target as Element).closest("[data-queue-trigger]")) {
                onClose();
            }
        };
    ...
    const { virtualItems, totalHeight } = useMemo(() => {
        ...
        // Keep the array in queue order, never in slot order. Slot order changes
        // during a drag, which makes React move the DOM nodes; a moved node drops
        // its in-flight `top` transition, so the rows that shift upward (dragging
        // down) snapped while the ones shifting downward animated. With a stable
        // order only `top` changes and every direction animates.
        if (drag) {
    ...
    // The panel's morph writes a transform on its content, and a transformed
    // ancestor makes `position: fixed` resolve against that ancestor: the ghost's
    // viewport `left` was then applied inside the panel and the card jumped to the
    // right. Portalling it to <body> keeps it in viewport coordinates.
    const ghost = drag ? (
        <div
            ref={ghostRef}
        ...
        >
                    {/* Same liquid glass as the panels: the lens runs on the card
                        itself, so the dragged row refracts the page under it. */}
                    <div className="glass-surface relative isolate flex h-[66px] scale-[1.02] items-center gap-3 rounded-2xl px-2 shadow-[0_24px_50px_rgba(0,0,0,0.35)]">
