...
import { publishAudioLevel, subscribeAudioDemand } from "@aura-music/core/audioLevelBridge";
...
const Visualizer: React.FC<VisualizerProps> = ({ audioRef, isPlaying, visible = true }) => {
    ...
    const demand = useRef(false);
    enabledRef.current = enabled;

    useEffect(() => subscribeAudioDemand((value) => {
        demand.current = value;
        workletNodeRef.current?.port.postMessage({ type: "BEAT", enabled: value && enabledRef.current });
    }), []);

    // Effect 1: Audio Context and Worklet Initialization
    useEffect(() => {
        workletNodeRef.current?.port.postMessage({ type: "BEAT", enabled: enabled && demand.current });
        if (!enabled) {
            publishAudioLevel(0);
        }
    }, [enabled]);
        ...
        const initAudio = async () => {
            ...
            if (!workletNodeRef.current) {
                try {
                    ...
                    workletNode.port.postMessage({ type: "BEAT", enabled: enabledRef.current && demand.current });
    ...
    useEffect(() => {
        ...
        try {
            const worker = new Worker(new URL("./VisualizerWorker", import.meta.url), {
                type: "module"
            });
