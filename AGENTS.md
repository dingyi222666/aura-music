...
- Group imports roughly in this order:
  ...
  2. Third-party packages (`react`, `@react-spring/web`, `fast-xml-parser`).
...
- Vite config lives in `vite.config.ts` and uses `loadEnv` for build-time
  settings such as `VITE_BASE_PATH`.
